...
$edit_default_storage_url = ControllersManager::getMenuLink(
    ...
    null,
    array(
        'inner_page' => StoragePageController::INNER_PAGE_EDIT_DEFAULT,
    )
);
        ...
        <tbody>
            ...
            foreach ($storages as $storage) :
                ...
                <tr id="main-view-<?php echo $storage->id ?>"
                    ...
                    data-name="<?php echo esc_attr($storage->name); ?>"
                    ...
                    <td>
                        <?php if ($storage->editable) : ?>                                                
                            <a href="javascript:void(0);" onclick="DupPro.Storage.Edit('<?php echo $storage->id; ?>')">
                                <b><?php echo esc_html($storage->name); ?></b>
                            </a>
                            <div class="sub-menu">
                                <a href="javascript:void(0);" onclick="DupPro.Storage.Edit('<?php echo $storage->id; ?>')">
                                    <?php DUP_PRO_U::esc_html_e('Edit'); ?>
                                </a> |
                                <a href="javascript:void(0);" onclick="DupPro.Storage.View('<?php echo $storage->id; ?>');">
                                    <?php DUP_PRO_U::esc_html_e('Quick View'); ?>
                                </a> |
                                <a href="javascript:void(0);" onclick="DupPro.Storage.CopyEdit('<?php echo $storage->id; ?>');">
                        ...
                        <?php else : ?>
                            ...
                            <div class="sub-menu">
                                <a href="javascript:void(0);" onclick="DupPro.Storage.EditDefault()">
                                    <?php DUP_PRO_U::esc_html_e('Edit'); ?>
                                </a> |
                                <a href="javascript:void(0);" onclick="DupPro.Storage.CopyEdit('<?php echo $storage->id; ?>');">
                                    <?php DUP_PRO_U::esc_html_e('Copy'); ?>
...
jQuery(document).ready(function($) {
    ...
    DupPro.Storage.deleteConfirm = function(idList) {
        ...
        idList.forEach(v => {
            $rowData    = $('#main-view-' + v);
            name        = $("<div/>").text($rowData.data('name')).html();
            id          = $rowData.data('id');
