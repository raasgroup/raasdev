...
// 						if (qlen == 0) { updraft_actually_proceeding = false;} 
					...
					function updates_intercept(e, passthis, checklink, via_shiny_updates, type) {
						// prevent the update from failing due to the absence of the "update-link" class
						if (!jQuery(passthis).hasClass('open-plugin-details-modal') && !jQuery(passthis).hasClass('update-link')) jQuery(passthis).addClass('update-link');
...
// 							link = '';
						...
						updraft_inpage_modal_buttons[updraftlion.proceedwithupdate] = function() {
							var update_deferred = jQuery.Deferred();
							...
							if (jQuery('#updraft_autobackup').is(':checked')) {
								jQuery.when(update_deferred).done(function() {
