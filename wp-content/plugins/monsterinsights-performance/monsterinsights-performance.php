...
class MonsterInsights_Performance {
	...
	private function check_compatibility() {
		...
		$compatibility = MonsterInsights_Performance_Compatibility_Check::get_instance();
	...
	public function modify_autoupdater_setting_html( $html, $plugin_file ) {
		if ( plugin_basename( __FILE__ ) === $plugin_file &&
			...
			$html = sprintf(
				'<a href="%s">%s</a>',
				'https://www.monsterinsights.com/docs/go-lite-pro/?utm_source=liteplugin&utm_medium=plugins-autoupdate&utm_campaign=upgrade-to-autoupdate&utm_content=monsterinsights-performance',
				__( 'Enable the MonsterInsights PRO plugin to manage auto-updates', 'monsterinsights-performance' )
			);
		}
	...
	public function require_frontend() {
		...
		// Local gtag.js
		require plugin_dir_path( __FILE__ ) . 'includes/frontend/gtag-local/class-gtag-js.php';
	}
	...
	public function requires_monsterinsights_version() {
		?>
		<div class="error">
			<p><?php esc_html_e( 'Please install or update MonsterInsights Pro with version 7.4 or newer to use the MonsterInsights Performance addon', 'monsterinsights-performance' ); ?></p>
		</div>
		<?php
	}
