...
<div class="container-wrap">
	...
	<div class="container main-content" data-col-num="cols-<?php echo esc_attr( $cols ); ?>">
		...
		<div id="portfolio" class="row portfolio-items <?php if ( $masonry_layout == 'true' ) { echo 'masonry-items'; } else { echo 'no-masonry'; } ?> <?php echo esc_attr( $infinite_scroll_class ); ?>"  data-categories-to-show="" data-starting-filter="" data-gutter="<?php echo esc_attr( $item_spacing ); ?>" data-masonry-type="<?php echo esc_attr( $masonry_sizing_type ); ?>" data-ps="<?php echo esc_attr( $project_style ); ?>" data-col-num="<?php echo esc_attr( $cols ); ?>">
			...
			if ( have_posts() ) :
				...
				<div class="col <?php echo esc_attr( $span_num ) . ' ' .  esc_attr( $masonry_item_sizing ); ?> element" data-project-cat="<?php echo esc_attr( $project_cats ); ?>" <?php if ( ! empty( $project_accent_color ) ) { echo 'data-project-color="' .  esc_attr( $project_accent_color ) . '"'; } else { echo 'data-default-color="true"';} ?> data-title-color="<?php echo esc_attr( $project_title_color ); ?>" data-subtitle-color="<?php echo  esc_attr( $project_subtitle_color ); ?>">
								...
								<div class="work-info"> 
									...
									<div class="vert-center">
										...
										// image
											...
											echo '<a href="' . esc_url( $featured_image[0] ) . '" class="pretty_photo default-link" >' . esc_html__( 'View Larger', 'salient-portfolio' ) . '</a> ';
										}
					...
					elseif ( $project_style === '2' ) {
						?>
							...
							<div class="work-item style-2">
										...
										<div class="work-info">
											...
											<a href="<?php echo esc_url( $the_project_link ); // WPCS: XSS ok. ?>"></a>
				
											<div class="vert-center"><h3><?php echo get_the_title(); ?></h3>
												<p>
													<?php
													if ( ! empty( $options['portfolio_date'] ) && $options['portfolio_date'] == 1 ) {
														echo get_the_date();
						...
						elseif ( $project_style === '6' ) {
							...
							<div class="work-item style-5" data-custom-content="<?php echo esc_url( $using_custom_content ); ?>" >
				...
				<div class="work-item style-2" data-custom-content="<?php echo esc_url( $using_custom_content ); ?>">
								...
								<div class="work-info">
										...
										if ( $using_custom_content == 'on' ) {
											if ( ! empty( $custom_project_link ) ) {
												echo '<a href="' . esc_url( $the_project_link ) . '"></a>'; // WPCS: XSS ok
											}
											// default
										} else {
											?>
							...
							<div class="work-item style-1" data-custom-content="<?php echo esc_attr( $using_custom_content ); ?>">
								...
								if ( ! empty( $custom_thumbnail ) ) {
									...
									if ( has_post_thumbnail() ) {
										...
										echo wp_kses_post( $project_featured_img ); // WPCS: XSS ok
									}
									// no image added
									else {
										switch ( $thumb_size ) {
										 ...
										 echo '<img src="' . SALIENT_PORTFOLIO_PLUGIN_PATH . '/img/' . esc_attr( $no_image_size )  . '" class="skip-lazy" alt="no image added yet." />';
									}
								}
								?>
		...
		if ( ! empty( $options['portfolio_extra_pagination'] ) && $options['portfolio_extra_pagination'] == '1' ) {
		   ...
		   $format              = empty( $permalink_structure ) ? '&paged=%#%' : 'page/%#%/';
			...
			if ( $total_pages > 1 ) {
				...
				echo '<div id="pagination" class="' . $fw_pagination . ' ' . $masonry_padding . esc_attr( $infinite_scroll_class ) . '" data-is-text="' . esc_attr__( 'All items loaded', 'salient-portfolio' ) . '">'; // WPCS: XSS ok.
		...
		// regular pagination
			...
			if ( get_next_posts_link() || get_previous_posts_link() ) {
				...
				echo '<div id="pagination" class="' . $fw_pagination . ' ' . $masonry_padding . esc_attr( $infinite_scroll_class ) . '" data-is-text="' . esc_attr__( 'All items loaded', 'salient-portfolio' ) . '">
