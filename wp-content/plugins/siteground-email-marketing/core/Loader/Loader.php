...
use SG_Email_Marketing\Integrations\ThirdParty\NinjaForms\NinjaForms as NinjaForms_Integration;
use SG_Email_Marketing\Integrations\ThirdParty\GravityForms\GravityForms as GravityForms_Integration;
...
class Loader {
	...
	private function load_dependencies() {
		...
		$this->integrations->ninja_forms                     = new NinjaForms_Integration( $this->mailer_api );
		$this->integrations->gravity_forms                   = new GravityForms_Integration( $this->mailer_api );
	...
	private function add_hooks() {
		...
		$this->add_ninja_forms_integration_hooks();
		$this->add_gravity_forms_integration_hooks();
		$this->add_cron_hooks();
		$this->add_form_hooks();
	...
	/**
	 * Add Ninja Forms integration hooks.
	 *
	 * @since 1.3.0
	 */
	public function add_ninja_forms_integration_hooks() {
		add_action( 'ninja_forms_register_actions', array( $this->integrations->ninja_forms, 'integrate_action' ) );
		add_action( 'ninja_forms_register_fields', array( $this->integrations->ninja_forms, 'integrate_field' ) );
		add_action( 'admin_enqueue_scripts', array( $this->integrations->ninja_forms, 'enqueue_editor_styles' ), 11 );
	}

	/**
	 * Add Gravity Forms integration hooks.
	 *
	 * @since 1.5.0
	 */
	public function add_gravity_forms_integration_hooks() {
		add_action( 'gform_field_standard_settings', array( $this->integrations->gravity_forms, 'sgwpmail_settings' ), 10, 2 );
		add_action( 'gform_editor_js', array( $this->integrations->gravity_forms, 'editor_script' ), 10, 2 );
		add_action( 'gform_after_submission', array( $this->integrations->gravity_forms, 'post_submission' ), 10, 2 );
		add_filter( 'gform_tooltips', array( $this->integrations->gravity_forms, 'sgwpmail_tooltips' ) );
		add_action( 'gform_editor_js_set_default_values', array( $this->integrations->gravity_forms, 'sgwpmail_set_defaults' ) );
		add_action( 'admin_enqueue_scripts', array( $this->integrations->gravity_forms, 'enqueue_editor_styles' ) );
		add_action( 'gform_pre_enqueue_scripts', array( $this->integrations->gravity_forms, 'enqueue_editor_styles' ) );
	}
}
