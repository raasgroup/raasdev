  ...
  <div class="pricing-table">
    <div class="plan featured" style="background: <?php echo esc_attr($price_bg); ?>; border: 2px solid <?php echo esc_attr($top_bg); ?>; transform: scale(1.0<?php echo esc_attr($zoom); ?>);">
      ...
      <div class="plan-select">
        <a href="<?php echo esc_url($btn_url['url']); ?>" target="<?php echo esc_attr($btn_url['target']); ?>" title="<?php echo esc_html($btn_url['title']); ?>" style="font-size: <?php echo esc_attr($btnsize); ?>;background: <?php echo esc_attr($top_bg); ?>;"><?php echo esc_attr($btn_text); ?></a>
      </div>
    </div>
  </div>
</div>