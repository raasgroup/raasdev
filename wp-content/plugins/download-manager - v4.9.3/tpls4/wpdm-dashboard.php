...
<div class="w3eden user-dashboard">
    <div class="media">
        <div id="wpdm-dashboard-sidebar">
            ...
            <div id="logo-block">
                <img style="margin-bottom: 10px;border-radius: 4px" class="thumbnail shop-logo" id="shop-logo" src="<?php echo isset($store['logo']) && $store['logo'] != '' ? $store['logo'] : get_avatar_url( $current_user->user_email, 512 ); ?>"/>
            </div>
            <div id="tabs">
                <?php
                foreach($this->dashboard_menu as $section_id => $section){
                    ...
                    foreach($section['items'] as $page_id => $menu_item){
                        ...
                        ?>
                        <a class="udb-item <?php echo $udb_page == $page_id?'selected':'';?>" href="<?php echo $menu_url; ?>"><i class="<?php echo $default_icons[$page_id]; ?> mr-3"></i><?php echo $menu_item['name']; ?></a>
                    <?php }
                    echo "</div>";
                } ?>
