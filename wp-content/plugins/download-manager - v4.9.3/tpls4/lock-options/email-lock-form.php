...
<form id="<?php echo $form_id; ?>" class="<?php echo $form_id; ?>" method=post action="<?php echo home_url('/'); ?>" style="font-weight:normal;font-size:12px;padding:0px;margin:0px">
    <div class="card card-default">
        ...
        <div class="card-body">
            ...
            <label><?php echo apply_filters("send_download_link_to_label",__( "Send Download Link To:" , "download-manager" ), $package['ID']); ?></label>
            <input type="email" required="required" class="form-control group-item email-lock-mail" placeholder="<?php _e("Email Address", "download-manager"); ; ?>" size="20" id="email_<?php echo $field_id; ?>" name="email" style="margin:5px 0" />



        </div><div class="card-footer text-right"><button id="wpdm_submit_<?php echo $field_id; ?>" class="wpdm_submit btn btn-success  group-item"  type=submit><?php echo $button_label; ?></button></div></div>
</form>
</div>
            ...
            $(this).ajaxSubmit({
                ...
                success:function(res){
                    ...
                    if( res.downloadurl ) {
                        ...
                        var html =
                            WPDM.overlay(".<?php echo $section_id; ?>", res.msg+"<a target=_blank style=\'margin-top:5px;color:#fff !important\' class=\'btn btn-success btn-lg btn-block\' href=\'"+res.downloadurl+"\'><?php echo $button_label; ?></a>");
                    } else {
                        WPDM.notify(res.error, 'error');
                    }
