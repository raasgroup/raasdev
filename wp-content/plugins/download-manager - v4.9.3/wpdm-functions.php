    ...
    if (isset($_POST['verify']) && $_POST['verify'] == 'email' && $file['email_lock'] == 1) {
        if (wpdm_verify_email($_POST['email'])) {
            ...
            if($idl === 0 || ($idl == 1 && $idle == 0)) {
                ...
                \WPDM\Email::send("email-lock", array('to_email' => $_POST['email'], 'name' => $name,  'package_name' => $file['post_title'], 'package_url' => get_permalink($id), 'download_url' => $download_url));
...
function wpdm_downloadable($content)
{
    ...
    $login_msg = get_option('wpdm_login_msg') ? stripcslashes(get_option('wpdm_login_msg')) : "<a href='" . get_option('siteurl') . "/wp-login.php'>Please login to access downloadables</a>";
...
function wpdm_embed_category($params = array('id' => '', 'operator' => 'IN' , 'items_per_page' => 10, 'title' => false, 'desc' => false, 'order_field' => 'create_date', 'order' => 'desc', 'paging' => false, 'toolbar' => 1, 'template' => '','cols'=>3, 'colspad'=>2, 'colsphone' => 1, 'morelink' => 1))
{
    extract($params);
    $fnparams = $params;
    if(!isset($id)) return;
