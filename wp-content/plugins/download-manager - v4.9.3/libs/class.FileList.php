...
//        ->Fetch('file-list.php');
            ...
            if (is_array($dfiles)) {
                ...
                foreach ($dfiles as $fileID => $sfile) {
                    ...
                    if ($idvdl == 1 && ($pwdlock || !$olock)) {
                        ...
                        if ($fileinfo[$sfile]['password'] != '' && $pwdlock)
                            $fhtml .= "<td style='width: 160px;white-space: nowrap;'><button class='inddl btn btn-primary btn-sm' data-pid='{$file['ID']}' data-file='{$fileID}' rel='" . $download_url . "&ind=" . $ind . "' data-pass='#pass_{$file['ID']}_{$ind}'><i class='fa fa-download'></i> &nbsp;".$button_label."</button>&nbsp;{$individual_file_actions}</td></tr>";
                        else {
                            $ind_download_link = "<a rel='nofollow' class='inddl btn btn-primary btn-sm' href='" . $download_url . "&ind=" . $ind . "'>".$button_label."</a>";
                            $ind_download_link = apply_filters("wpdm_single_file_download_link", $ind_download_link, $fileID,  $package);
                            $fhtml .= "<td style='width: 160px;white-space: nowrap;' class='text-right'>{$ind_download_link}{$play}&nbsp;{$individual_file_actions}</td></tr>";
     ...
     */
    public static function Box($file, $h = 88, $w = 88, $cols = 3)
    {
        global $current_user;
        ...
        if (count($file['files']) > 0 || count($dfiles) > 0) {
            ...
            if (is_array($allfiles)) {
                ...
                foreach ($allfiles as $fileID => $sfile) {
                    $fhtml .= "<div class='{$class} col-sm-6 col-xs-6'><div class='panel panel-default'>";
                    $ind = $fileID; //\WPDM_Crypt::Encrypt($sfile);
                    ...
                    if(in_array($ext, $imgext))
                        $thumb = wpdm_dynamic_thumb($filepath, array($h, $w));
                    if($thumb)
        ...
        if (count($file['files']) > 0) {
            ...
            if (is_array($allfiles)) {
                ...
                foreach ($allfiles as $fileID => $sfile) {
                    ...
                    if(Package::userCanDownload($file['ID'])){
                        $fhtml .= "<label class='list-group-item eden-checkbox free-file'><div class='pull-right'><a class='wpdm-download-link ".wpdm_download_button_style(null, $file['ID'])."' href='#'>".__("Download", "download-manager")."</a></div> $fileTitle</label>";
                    } else {
