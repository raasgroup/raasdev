...
class PackageLocks
{
    ...
    public static function linkedInShare($package)
    {
        ...
        return "<button class='wpdm-social-lock btn wpdm-linkedin' data-url='".SocialConnect::LinkedinAuthUrl($package['ID'])."'><i class='fab fa-linkedin-in'></i> Share</button>";
    ...
    public static  function askEmail($package)
    {
        ...
        if (isset($package['email_lock']) && $package['email_lock'] == '1') {
            ...
            $lock = 'locked';
            $unqid = uniqid();
            $section_title = isset($package['email_lock_title']) && $package['email_lock_title'] != ''? $package['email_lock_title']:__( "Subscribe To Download" , "download-manager" );
    ...
    public static function facebookLike($package, $buttononly = false)
    {
        ...
        return "<button class='wpdm-social-lock btn wpdm-facebook' data-url='".SocialConnect::FacebookLikeUrl($package['ID'])."'><i class='fab fa-facebook-f'></i> Like</button>";
