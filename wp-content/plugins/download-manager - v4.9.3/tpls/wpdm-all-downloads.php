...
$p = serialize($params);
$tid = md5($p);
if(!is_array($params)) $params = array('items_per_page' => 20);
...
<style>
    ...
    #wpdmmydls-<?php echo $tid; ?>_length select.wpdm-custom-select{
            ...
            while ($q->have_posts()): $q->the_post();
                ...
                if(isset($data['files'])&&is_array($data['files'])){
                    if(count($data['files']) == 1) {
                        ...
                        $tmpvar = $tmpvar?explode(".", $tmpvar):array();
                ...
                if(class_exists('WPSEO_Primary_Term')) {
                    $wpseo_primary_term = new WPSEO_Primary_Term('wpdmcategory', get_the_ID());
                    $wpseo_primary_term = $wpseo_primary_term->get_primary_term();
                    $primary_cat = get_term($wpseo_primary_term);
                }
                foreach($cats as $cat){
                    //if(!isset($primary_cat) || $primary_cat->term_id != $cat->term_id)
                        ...
                        <td <?php if(in_array($cold[0], array('publish_date', 'update_date','create_date'))) { ?> data-order="<?php echo $dor[$cold[0]]; ?>" <?php } ?> class="__dt_col_<?php echo $colx; ?> __dt_col __dt_col_<?php echo $cold[0]; ?>" <?php if($colx == 0) { ?>style="background-image: url('<?php echo $ext ; ?>');background-size: 36px;background-position: 5px 8px;background-repeat:  no-repeat;padding-left: 52px;line-height: normal;"<?php } ?>>
                            ...
                            foreach ($cold as $cx => $c){
                                ...
                                switch ($c) {
                                    ...
                                    case 'file_count':
                                        if($cx > 0)
                                            echo "<span class='__dt_file_count {$cxc}'><i class=\"fas fa-copy\"></i> ". count($data['files'])." " . __('file(s)','download-manager')."</span>";
                                        else
                                    ...
                                    case 'primary_cat':
                                        if(class_exists('WPSEO_Primary_Term')) {
                                            ...
                                            $pcurl = add_query_arg(array('wpdmc' => $term->slug), get_permalink($pageid));
                                            echo "<span class='__dt_primary_cat'><a href='{$pcurl}'>" . $term->name . "</a></span>";
                                        }
                                        break;
                                    case 'categories':
