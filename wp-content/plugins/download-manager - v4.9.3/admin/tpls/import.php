...
<div class="col-md-3">
                ...
                <script type="text/javascript">
                    ...
                    jQuery(function($){
                        ...
                        $('#xportfile').on('click', function () {
                            $(this).hide();
                            $('#wxprogressbar').show();
                            $.get(ajaxurl, {action: 'wpdm_export_packages', _csvexport_nonce: '<?php echo wp_create_nonce(NONCE_KEY); ?>', format: 'csv'}, function (data) {
                    ...
                    function WPDM_Create_Export_File(key) {
                        var $ = jQuery;
                        $.get(ajaxurl, {action: 'wpdm_export_packages', _csvexport_nonce: '<?php echo wp_create_nonce(NONCE_KEY); ?>', _key: key}, function (res) {
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body" id="wxstats">
            <label><input type="radio" checked="checked" value="1"> CSV</label>
        </div>
  ...
  <table class="table table-hover">
	...
	for($index=$s; $index<$max; $index++): $value = $fileinfo[$index]; $tmptitle = ucwords(str_replace(array("-","_",".")," ",$value['name'])); ?>
      <tr for="file-<?php echo $index; ?>" valign="top" class="importfilelist" id="<?php echo $index; ?>">
        <th    class=" check-column" style="border-right: 1px solid #dbdbdb;padding-bottom: 0px;"><input type="checkbox" rel="<?php echo $index; ?>" id="file-<?php echo $index; ?>" class="checkbox dim" value="<?php echo $value['name'] ?>"></th>
...
<script type="text/javascript">
    ...
    jQuery(function($) {
        ...
        $('#idel,#idel1').click(function(){
            $('.dim').each(function(){
                if(this.checked)
                    dimport($(this).attr('rel'),$(this).val());
            });
        });
