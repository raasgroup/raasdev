        ...
        if( count($files) > 2 ) { /* The 2 accounts for . and .. */
            ...
            foreach( $files as $file ) {
                if( file_exists($root . $_POST['dir'] . $file) && $file != '.' && $file != '..' && is_dir($root . $_POST['dir'] . $file) ) {
                    echo "<li class=\"directory collapsed\"><a onclick=\"odirpath(this)\" class=\"odir\" href=\"#\" rel=\"" . htmlentities(realpath($_POST['dir'] . $file)) . "/\">" . htmlentities($file) . "</a></li>";
                }
            }
            echo "</ul>";
        }
        ...
        <div class="col-md-12 selc"><br/>
            <div class="input-group"><input type="text" id="dpath" class="form-control">
                <div class="input-group-btn">
                    <button type="button" id="slctdir" class="btn btn-secondary">Insert</button>
                </div>
            </div>
            <br/>
...
function wpmp_dir_browser_metabox($post){
    ...
    <input class="form-control" type="text" id="srvdir" value="<?php echo get_post_meta($post->ID,'__wpdm_package_dir', true); ?>" name="file[package_dir]" />
            <div class="input-group-btn">
    <a href="admin.php?post_type=wpdmpro&task=wpdm_dir_browser" title="Server File Browser" class="thickbox btn btn-secondary"><i class="fa fa-folder-open"></i></a>
    </div>
</div> </div>
