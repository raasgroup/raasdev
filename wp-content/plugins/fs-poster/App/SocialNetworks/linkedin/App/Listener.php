...
class Listener
{
	...
	public static function sharePost( $result, $postData )
	{
		...
		if ( $postData->sendType != 'image' && $postData->sendType != 'video' )
		{
			...
			if ( $pMethod === 1 )
			{
				$thumbnailPath = $postData->getPostThumbnailURL();
			...
			{
				$imagesLocale = $postData->getPostGalleryURL();
