...
class Listener
{
	...
	private static function cookieMethod( $postData )
	{
		...
		$productPrice    = $gbShareAsProduct ? Helper::getProductPrice( $postData->post, 'price', FALSE ) : NULL;
		...
		$google = new GoogleMyBusiness( $cookieSID, $cookieHSID, $cookieSSID, $postData->getProxy() );

		return $google->sendPost( $postData->getNodeProfileId(), $postData->message, $postData->link, $linkButton, $imageUrl, $productName, $productPrice, $productCurrency, $productCategory );
	}
