...
class Listener
{
	...
	public static function sharePost( $result, $postData )
	{
		...
		else if ( $postData->sendType !== 'image' )
		{
			...
			{
				$postImages = array_slice( $postData->getPostGalleryURL(), 0, 10, TRUE );
		...
		{
			return $planly->post( $postData->message, $postData->sendType, $media, $postData->node[ 'info' ][ 'access_token' ], $postData->getNodeProfileId(), $postData->nodeType );
		}
