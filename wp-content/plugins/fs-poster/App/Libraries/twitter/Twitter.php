...
class Twitter extends SocialNetwork
{
	...
	public static function sendPost ( $appId, $type, $message, $comment, $username, $link, $images, $video, $accessToken, $accessTokenSecret, $proxy )
	{
		...
		if ( $type === 'image' && ! empty( $images ) && is_array( $images ) )
		{
			...
			{
				$parameters[ 'media_ids' ] = implode( ',', $uploadedImages );
			}
		}
		...
		if ( $type === 'video' && ! empty( $video ) && is_string( $video ) )
		{
			...
			{
				$parameters[ 'media_ids' ] = $uploadVideo[ 'media_id_string' ];
			}
		}

		$result = $connection->post( 'https://api.twitter.com/1.1/statuses/update.json', $parameters );
		...
		else if ( ! empty( $result[ 'id_str' ] ) )
		{
			$mediaId = $result[ 'id_str' ];

			if ( ! empty( $comment ) )//post a comment
			{
				$connection->post( 'https://api.twitter.com/1.1/statuses/update.json', [
					'status'                => sprintf( "@%s \n %s", $username, $comment ),
					'in_reply_to_status_id' => $mediaId
				] );
			}
