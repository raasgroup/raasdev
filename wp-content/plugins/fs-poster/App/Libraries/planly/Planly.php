...
class Planly
{
	...
	public function post ( $message, $type, $media, $teamId, $channelId, $sn )
	{
        ...
        {
            $media = $this->upload( $url, $type, $teamId );
	...
	private function upload ( $url, $type, $teamId )
	{
		$media = file_get_contents( $url );

		$contentType = ( new finfo( FILEINFO_MIME_TYPE ) )->buffer( $media );
        ...
        if ( $type === "image" )
        {
            $response = $this->cmd( "media/upload-photo", $media, $headers );
        }
        else if ( $type === "video" )
        {
            $response = $this->cmd( "media/upload-video", $media, $headers );
        }
        else
        {
            return [
                "status"    => "error",
                "error_msg" => "Invalid media type: {$type}"
            ];
        }
