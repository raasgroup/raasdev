...
class Linkedin extends SocialNetwork
{
	...
	private static function scrapeURL ( $url, $author, $accessToken, $proxy )
	{
		...
		{
			$uploadThumb = self::uploadImages( [ $scraped[ 'image' ] ], $author, $accessToken, $proxy, FALSE );
	...
	private static function uploadImages ( $images, $author, $accessToken, $proxy, $isLocalFile = TRUE )
	{
		$send_upload_data = [
		...
		foreach ( $images as $imageURL )
		{
			try
			{
				...
				$resp = $client->request( 'PUT', $uploadURL, [
					'body'    => $isLocalFile ? file_get_contents( $imageURL ) : Curl::getURL( $imageURL ),
					'headers' => [
						'Authorization' => 'Bearer ' . $accessToken,
	...
	private static function uploadVideo ( $owner, $file, $accessToken, $proxy )
	{
		...
		$initialData = [
			'initializeUploadRequest' => [
				'owner'           => $owner,
				'fileSizeBytes'   => strlen( Curl::getContents( $file ) ),
		...
		$fileContent = Curl::getContents( $file );
