...
class Pinterest extends SocialNetwork
{
	...
	public static function sendPost ( $boardId, $type, $title, $message, $link, $altText, $images, $accessToken, $proxy )
	{
		...
		$sendData[ 'media_source' ][ 'content_type' ] = Helper::mimeContentType( $image );
		$sendData[ 'media_source' ][ 'data' ]         = base64_encode( file_get_contents( $image ) );
	...
	public static function get_boards ( $accountId, $accessToken, $proxy )
	{
		...
		do
		{
			...
			if ( ! empty( $page[ 'items' ] ) )
			{
				...
				$bookmark = ! isset( $page[ 'bookmark' ] ) || $bookmark == $page[ 'bookmark' ] || count( $page[ 'page_items' ] ) < 100 ? NULL : $page[ 'bookmark' ];
			}
			else
			{
				break;
