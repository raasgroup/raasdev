		...
		if ( $scheduleId > 0 )
		{
			switch ( $schedule_info[ 'post_sort' ] )
			{
				...
				case "random2":
					...
					break;
				case "old_first":
					$last_shared_post = DB::DB()->get_row( 'SELECT `post_id`, `wp_post_date` FROM `' . DB::table( 'feeds' ) . '` WHERE `schedule_id` = "' . ( int ) $scheduleId . '" ORDER BY `id` DESC LIMIT 1', ARRAY_A );

					if ( ! empty( $last_shared_post[ 'post_id' ] ) )
					{
                        $last_post_date = empty( $last_shared_post[ 'wp_post_date' ] ) ? Date::dateTimeSQL( get_the_date( 'Y-m-d H:i:s', $last_shared_post[ 'post_id' ] ) ) : $last_shared_post[ 'wp_post_date' ];						if ( $last_post_date )
						{
