...
class ShareService
{
	public static function insertFeeds ( $wpPostId, $userId, $nodes_list, $custom_messages, $categoryFilter = TRUE, $schedule_date = NULL, $sharedFrom = NULL, $shareOnBackground = NULL, $scheduleId = NULL, $disableStartInterval = FALSE, $instagramPinThePost = 0 )
	{
		/**
		...
		foreach ( $nodes_list as $nodeId )
		{
			if ( is_string( $nodeId ) && strpos( $nodeId, ':' ) !== FALSE )
			{
				...
				$feedSQL = [
					...
					'driver'              => $driver,
					'post_id'             => $wpPostId,
					'wp_post_date'        => Date::dateTimeSQL( get_the_date( 'Y-m-d H:i:s', $wpPostId ) ),
					'node_type'           => $nodeType,
					'node_id'             => (int) $nodeId,
	...
	public static function doAutoReschedule ( $scheduleInfo )
	{
		...
		DB::DB()->update( DB::table( 'schedules' ), [ 'data' => json_encode( $data ) ], [ 'id' => $scheduleInfo[ 'id' ] ] );

		$data[ 'parentScheduleID' ] = $scheduleInfo[ 'id' ];
		$autoReschedulesDone        = $data[ 'autoReschdulesDone' ];

		unset( $scheduleInfo[ 'id' ], $data[ 'autoRescheduleEnabled' ], $data[ 'autoRescheduleCount' ], $data[ 'autoReschdulesDone' ] );

		$scheduleInfo[ 'title' ]       = 'Re#' . $autoReschedulesDone . ': ' . $scheduleInfo[ 'title' ];
		$scheduleInfo[ 'status' ]      = 'active';
		$scheduleInfo[ 'post_ids' ]    = $scheduleInfo[ 'save_post_ids' ];
		$scheduleInfo[ 'insert_date' ] = Date::dateTimeSQL();
		$scheduleInfo[ 'data' ] = json_encode( $data );

		DB::DB()->insert( DB::table( 'schedules' ), $scheduleInfo );
	}
	...
	public static function scheduledPost ( $schedule )
	{
		...
		self::insertFeeds( $post_id, $userId, $nodeList, $customPostMessages, false, NULL, 'schedule', 1, $scheduleId, TRUE, $instagramPinThePost );
